...
const ProductDetails = () => {
  ...
  const editProduct = (data) => {
    ...
    if (initialValues.price !== data.price) {
      formData.append("price", data.price);
    }

    if (initialValues.category !== data.category) {
      formData.append("category", data.category);
    }

    if (initialValues.quantity !== data.quantity) {
      ...
      <DefaultLayout>
        ...
        >
          ...
          <div className="my-8 flex items-center justify-center font-bold">
            <button
              className="mx-2 grow rounded-md bg-blue-500 py-2 px-20 text-white"
              type="submit"
            >
              Edit
            </button>
